...
setwd(dir = "/Users/audrey/Desktop/Attempt3_Pig2Mouse/Fecal")

ASVs <- read_qza("fecal-filtered-table.qza")
ASV_table <- as.data.frame(ASVs$data)

#####################################################################
##I added in this chunk. What does it do and why?
...
str(metadata)
colnames(metadata)[12] = "donortype"

dataset <- merge(metadata, dataset, by.x = "SampleID", by.y = 0)
treatments<-as.vector(unique(dataset$donortype))
...
summary(metadata$donortype)

my_column <- "donortype"
summary(dataset$donortype)
#Determine from number of samples in each treatment (ie. how many piglet recipients)
#Need at least 6 samples per treatment bare minimum
n1 <- 8
n2 <- 8
...
strong_results<-subset(final_results, rho >= 0.6)
...
head(final_results)
ggplot(gut_ASVs, aes(x = ASV104, y = ASV29)) +
  geom_point()
...
write.csv(strong_results_taxa, "output/pig2mouse-strong-results-taxa.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="Piglet"), "output/pig2mouse-strong-results-piglet-taxa.csv", row.names = F)
write.csv(subset(strong_results_taxa, trt=="Sow"), "output/pig2mouse-strong-results-sow-taxa.csv", row.names = F)
