...
physeq <- qza_to_phyloseq(
  features="fecal-core-metrics-results/rarefied_table.qza",
  tree="rooted-tree.qza",
  taxonomy = "taxonomy.qza",
  metadata = "replicate-metadata-gnoto.txt"
)
...
#metadata$donor.type.ord = factor(metadata$donor.type, c("Piglet", "Sow"))
...
#All of this is OK except that in future use of the taxonomy table, 
...
my_column <- "donor.type"  #this is the metadata column that we will use in the taxa barplot
...
###Don't mess with stuff after this point.
for(ml in my_level){
  print(ml)
  ...
  physeq_meta_filtered$donor.type.ord = factor(physeq_meta_filtered$donor.type, c("Piglet", "Sow"))
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ donor.type)
