...
bc_PCoA<-read_qza("donor-core-metrics-results/bray_curtis_pcoa_results.qza")
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=donor_type_colors, name = my_column)
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=donor.type, shape=replicate)) +
  ...
  xlab(paste0("PC1 (", round(100*bc_PCoA$data$ProportionExplained[1], digits = 2), "%)")) +
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  labs(shape = "Donor", color = "Donor Type")+
...
Wuni_PCoA<-read_qza("donor-core-metrics-results/weighted_unifrac_pcoa_results.qza")
...
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column), shape= replicate)) +
  geom_point(size = 3) + #alpha controls transparency and helps when points are overlapping
  #geom_point(data=centroids, size = 3) +
  theme_q2r() +
  #stat_ellipse(level = 0.95, type = "t") +
...
bc_dist_mat<-read_qza("donor-core-metrics-results/bray_curtis_distance_matrix.qza")
...
write.table(BC_PERMANOVA_out,"output/BC_donor.type_Adonis_overall.csv",sep=",", row.names = TRUE) 
...
UWuni_PCoA<-read_qza("donor-core-metrics-results/unweighted_unifrac_pcoa_results.qza")
...
Jaccard_PCoA<-read_qza("donor-core-metrics-results/jaccard_pcoa_results.qza")
...
##Weighted UniFrac##
wuni_dist_mat<-read_qza("donor-core-metrics-results/weighted_unifrac_distance_matrix.qza")
...
write.table(Wuni_PERMANOVA_out,"output/weighted_unifrac_donor.type_Adonis_overall.csv",sep=",", row.names = TRUE) 

##Unweighted UniFrac##
uwuni_dist_mat<-read_qza("donor-core-metrics-results/unweighted_unifrac_distance_matrix.qza")
uwuni_dm <- as.matrix(uwuni_dist_mat$data) 
rownames(uwuni_dm) == metadata$SampleID ## all these values need to be "TRUE"
### Not all values were true.
metadata_sub <- metadata[match(rownames(uwuni_dm),metadata$SampleID),]
rownames(uwuni_dm) == metadata_sub$SampleID ## all these values need to be "TRUE"
#All values are true.

UWuni_PERMANOVA_out <- adonis2(uwuni_dm ~ donor.type, data = metadata_sub)

write.table(UWuni_PERMANOVA_out,"output/unweighted_unifrac_donor.type_Adonis_overall.csv",sep=",", row.names = TRUE) 


##Jaccard##
jacc_dist_mat<-read_qza("donor-core-metrics-results/jaccard_distance_matrix.qza")
...
Jaccard_PERMANOVA_out <- adonis2(jacc_dm ~ donor.type, data = metadata_sub)

write.table(Jaccard_PERMANOVA_out,"output/jaccard_donor.type_Adonis_overall.csv",sep=",", row.names = TRUE) 
