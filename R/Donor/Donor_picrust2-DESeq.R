...
setwd("/Users/audrey/Desktop/Attempt3_Pig2Mouse/Donor")
list.files()

if(!dir.exists("picrust2_output"))
  dir.create("picrust2_output")
...
metadata <- metadata[,-1]
colnames(metadata)[11] = "donor.type"
levels(metadata$donor.type)
metadata$donor.type.ord = factor(metadata$donor.type, c("control", "donor-piglet", "donor-sow", "Piglet", "Sow"))
levels(metadata$donor.type.ord)
colnames(metadata)[1] <- "sample.type"
colnames(metadata)[13] <- "for.filtering"
str(metadata)
...
diagdds = phyloseq_to_deseq2(physeq_picrust, ~ donor.type)
...
alpha = 0.05
my_contrast = c("donor.type", "donor-piglet", "donor-sow") 
...
ggsave(paste0("picrust2_output/DESeq2-", my_contrast[2], "-", my_contrast[3], ".png"), DESeq_fig, height = 10, width = 40)
...
ggsave(paste0("picrust2_output/DESeq2-", my_contrast[2], "-", my_contrast[3], "-filtered.png"), DESeq_fig, height = 8, width = 15)
...
run_deseq2 <- function(my_factor, x, y){
  ...
  ggsave(paste0("picrust2_output/DESeq2-", my_contrast[2], "-", my_contrast[3], "-filt.png"), DESeq_fig, height = 10, width = 20)
}

run_deseq2("donor.type", "donor-piglet", "donor-sow")
